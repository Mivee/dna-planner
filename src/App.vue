<template>
  <div class="min-h-screen bg-primary">
    ...
    <header
      class="bg-gradient-to-r from-primary via-secondary to-primary border-b border-white/10 shadow-lg sticky top-0 z-[100] backdrop-blur-[10px]">
      <div class="max-w-[1400px] mx-auto px-6 py-4 flex items-center gap-8 flex-wrap">
        ...
        <nav class="flex gap-2 flex-1 flex-wrap" role="navigation" aria-label="Main navigation">
          <RouterLink to="/character"
            class="flex items-center gap-2 px-4 py-2.5 bg-white/5 rounded-md text-on-secondary font-semibold text-sm transition-all duration-200 ease-in-out whitespace-nowrap hover:bg-white/10 hover:text-on-primary [&.router-link-active]:bg-info [&.router-link-active]:text-white"
    ...
    <Modal v-model:isOpen="showSettings" @save="handleSaveSettings">
      <template #header>
        <h2 class="m-0 text-xl text-on-primary font-bold">Settings</h2>
      </template>
      <Settings />
    </Modal>
  </div>
