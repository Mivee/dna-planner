...
const { upgradeMaterials, buildSummary } = useCharacter(
	props.upgradeConfig.name!
);
const summary = computed(() =>
	buildSummary(characterAscensionMaterials.value, [
		...skillLevelMaterials.value.passive,
		...skillLevelMaterials.value.skill,
		...skillLevelMaterials.value.ult,
	])
);
const characterAscensionMaterials = computed(() => {
	const start = characterLevelingMaterials.find(
		(mat) => mat.level == props.upgradeConfig.level.start
	)!;
	const end = characterLevelingMaterials.find(
		(mat) => mat.level == props.upgradeConfig.level.end
	)!;
...
const skillLevelMaterials = computed(() => {
	...
	return {
		...
		ult: ultimateMaterials,
		passive: passiveMaterials,
	};
});
...
function mergeRange(range: LevelRange<SkillLevelCost>) {
	return {
		...
		forgingMaterials: {
			T1Green:
				range.end!.forgingMaterials.T1Green -
				range.start!.forgingMaterials.T1Green,
			T2Blue:
				range.end!.forgingMaterials.T2Blue -
				range.start!.forgingMaterials.T2Blue,
			T3Purple:
				range.end!.forgingMaterials.T3Purple -
				range.start!.forgingMaterials.T3Purple,
			T4Gold:
				range.end!.forgingMaterials.T4Gold -
				range.start!.forgingMaterials.T4Gold,
		},
...
function getMaterialName(color: materialColor, type: "Ascension" | "Forging") {
	if (type == "Ascension") {
		switch (color) {
			case "Green":
				return upgradeMaterials.value?.ascensionMaterials.t1;
			case "Blue":
				return upgradeMaterials.value?.ascensionMaterials.t2;
			case "Purple":
				return upgradeMaterials.value?.ascensionMaterials.t3;
			default:
				return null;
		}
		...
		switch (color) {
			...
			case "Purple":
				return upgradeMaterials.value?.forgingMaterials.t3;
			case "Gold":
				return upgradeMaterials.value?.forgingMaterials.t4;
			default:
				return null;
		}
	}
