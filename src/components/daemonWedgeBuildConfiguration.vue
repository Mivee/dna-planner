    ...
    <Modal v-model:isOpen="isOpen" :title="isEditing ? 'Edit Daemon Wedge' : 'Add Daemon Wedge'"
        ...
        <div class="space-y-4">
            <div>
                ...
                <select v-model="selectedWedge" :disabled="isEditing"
                    ...
                    <optgroup v-for="category in groupedWedges" :key="category.label" :label="category.label">
                        <option v-for="wedge in category.wedges" :key="wedge.id" :value="wedge.displayName">
                            {{ wedge.displayName }}
                        </option>
                    </optgroup>
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-2 text-white-muted">Initial Level</label>
                    <select v-model.number="initialLevel"
                        ...
                        <option v-for="level in 10" :key="level - 1" :value="level - 1">+{{ level - 1 }}</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2 text-white-muted">Target Level</label>
                    <select v-model.number="targetLevel"
                        class="w-full p-3 bg-secondary-light border border-white/20 rounded-lg text-white focus:border-accent focus:ring-1 focus:ring-accent transition-all">
                        <option v-for="level in 11" :key="level - 1" :value="level - 1"
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-white/20">
                ...
                <button @click="saveConfiguration" :disabled="!canAdd"
