    ...
    <div class="bg-secondary border border-white/10 rounded-lg overflow-hidden sticky top-4 h-fit">
        ...
        <div class="p-4 flex flex-col gap-6">
            <div class="flex flex-col gap-2">
                ...
                <div v-if="totalMaterials.carmineGlobules > 0"
                    class="flex items-center gap-3 p-2 bg-secondary/3 rounded-md hover:bg-secondary/5 transition-colors">
                    <i class="fas fa-gem w-5 text-center text-purple-400"></i>
                    <span class="flex-1 text-sm text-on-primary">Carmine Globules</span>
                    <span class="font-bold text-purple-400 text-sm">{{ getAdjustedQuantity('Carmine Globule',
                        totalMaterials.carmineGlobules).toLocaleString() }}</span>
                </div>
                <div
                    class="flex items-center gap-3 p-2 bg-secondary/3 rounded-md hover:bg-secondary/5 transition-colors">
            ...
            <div class="flex flex-col gap-2" v-if="hasBlueprints">
                <h4 class="text-xs font-semibold text-on-secondary uppercase tracking-wider m-0 mb-2">Blueprints</h4>
                <template v-for="(blueprint, key) in blueprintsList" :key="key">
                    <div
                        class="flex items-center gap-3 p-2 bg-secondary/3 rounded-md hover:bg-secondary/5 transition-colors">
                        <i class="fas fa-scroll w-5 text-center" :class="blueprint.colorClass"></i>
                        <span class="flex-1 text-sm text-on-primary">{{ blueprint.name }}</span>
                        <span class="font-bold text-accent text-sm">{{ blueprint.adjustedQuantity }}</span>
                    </div>
                </template>
            </div>

            <div class="flex flex-col gap-2" v-if="hasDaemonWedgeMaterials">
                <h4 class="text-xs font-semibold text-on-secondary uppercase tracking-wider m-0 mb-2">Daemon Wedge
                    Materials</h4>
                <template v-for="(material, key) in daemonWedgeMaterialsList" :key="key">
                    <div
                        class="flex items-center gap-3 p-2 bg-secondary/3 rounded-md hover:bg-secondary/5 transition-colors">
...
const totalMaterials = computed(() => {
    ...
    daemonWedgeConfigs.forEach(config => {
        ...
        try {
            ...
            // Get summary
            const summary = buildSummary(wedge, config.initialLevel, config.targetLevel, quantity);
            ...
            console.warn(`Failed to calculate materials for daemon wedge ${config.name}:`, error);
        }
    });
