        ...
        <div
            ...
            <div class="flex gap-2">
                <button
                    ...
                    @click="edit" aria-label="Edit daemon wedge">
                ...
                <button
                    ...
                    @click="remove" aria-label="Remove daemon wedge">
        ...
        <div class="p-5 border-b border-white/20">
            <div class="flex flex-col gap-3">
                ...
                <div v-if="config.quantity && config.quantity > 1" class="flex justify-between items-center py-2">
                    ...
                    <span class="text-sm font-bold text-blue-400">×{{ config.quantity }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="p-5" v-if="summary">
            <div class="flex flex-col gap-3">
                ...
                <div class="flex justify-between items-center py-2">
                    ...
                    <span class="text-sm font-bold text-accent">{{ summary.coins.toLocaleString() }}</span>
                </div>
                ...
                <!-- Blueprints -->
                <div v-if="summary.blueprints.size > 0" class="border-t border-white/10 pt-3 mt-2">
                    <h4 class="text-xs font-semibold text-white-muted uppercase mb-2">Blueprints</h4>
                    <div v-for="[name, qty] in summary.blueprints" :key="name"
                        class="flex justify-between items-center py-1">
                        <div class="flex items-center gap-2">
                            <i class="fas fa-scroll w-5" :class="getBlueprintColor(name)"></i>
                            <span class="text-xs text-white-muted">{{ name }}</span>
                        </div>
                ...
                <!-- Materials -->
                <div v-if="materialsArray.length > 0" class="border-t border-white/10 pt-3 mt-2">
                    <h4 class="text-xs font-semibold text-white-muted uppercase mb-2">Materials</h4>
                    <div v-for="material in materialsArray" :key="material.name"
                        class="flex justify-between items-center py-1">
                        <div class="flex items-center gap-2">
                            <i class="fas fa-cube text-accent w-5"></i>
                            <span class="text-xs text-white-muted">{{ material.name }}</span>
                        </div>
                        <span class="text-xs font-bold">{{ material.quantity }}</span>
                    </div>
                </div>
            </div>
        </div>

        <DaemonWedgeBuildConfiguration v-if="isEditing" :upgrade-config="props.config" @saved="toggleIsEditing"
...
const summary = computed(() => {
    ...
    return buildSummary(wedge.value, props.config.initialLevel, props.config.targetLevel, quantity);
});

const materialsArray = computed(() => {
    if (!summary.value) return [];
    return Array.from(summary.value.materials.entries()).map(([name, quantity]) => ({
        name,
        quantity
    }));
});
